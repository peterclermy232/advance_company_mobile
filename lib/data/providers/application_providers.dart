...
// Single request → both application types (for the form dropdown)
// and status choices (for filters / list card colours).
...
class ApplicationsNotifier extends AsyncNotifier<List<ApplicationModel>> {
  ...
  Future<void> approveApplication(int id, {String comments = ''}) async {
    final apiClient = await ref.read(apiClientProvider.future);
    await apiClient.post(
  ...
  Future<void> rejectApplication(int id, {String comments = ''}) async {
    final apiClient = await ref.read(apiClientProvider.future);
    await apiClient.post(
  ...
  Future<void> markUnderReview(int id) async {
    final apiClient = await ref.read(apiClientProvider.future);
...
FutureProvider.family<ApplicationModel, int>((ref, id) async {
  final apiClient = await ref.read(apiClientProvider.future);
